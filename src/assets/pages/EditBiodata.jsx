...
function EditBiodata() {
  ...
  const { id } = useParams()
  const { getBiodataById, updateBiodata, calculateAge, selectedTemplate, setSelectedTemplate } = useBiodata()
  ...
  useEffect(() => {
    const biodata = getBiodataById(id)
    if (biodata) {
      setFormData(biodata)
      setPhotoPreview(biodata.profilePhoto || '')
      setSelectedTemplate(biodata.template || 'classic')
    } else {
      navigate('/dashboard')
    }
  }, [id])
  ...
  const handleSubmit = (e) => {
    ...
    updateBiodata(parseInt(id), biodata)
    setSaving(false)
    navigate('/dashboard')
  }
